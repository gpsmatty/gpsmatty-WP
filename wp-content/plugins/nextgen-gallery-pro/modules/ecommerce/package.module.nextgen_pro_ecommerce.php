...
class A_Manual_Pricelist_Form extends Mixin
{
    ...
    function _render_manual_pricelist_items_field()
    {
        ...
        $manual_pricelist_template = 'photocrati-nextgen_pro_ecommerce#manual_pricelist';
        /* Use redesigned template if user has redesign version of NGG */
        if (defined('NGG_PLUGIN_VERSION')) {
            if (version_compare(NGG_PLUGIN_VERSION, '2.2.99') >= 0) {
                $manual_pricelist_template = 'photocrati-nextgen_pro_ecommerce#manual_pricelist_redesign';
            }
        }
        return $this->object->render_partial($manual_pricelist_template, array('items' => $items, 'manual_settings' => $this->get_model()->manual_settings, 'i18n' => $this->get_i18n_strings(), 'shipping_methods' => $this->object->get_shipping_methods(), 'item_source' => NGG_PRO_MANUAL_PRICELIST_SOURCE), TRUE);
    }
...
class C_Pricelist_Source_Page extends C_NextGen_Admin_Page_Controller
{
    ...
    function index_template()
    {
        ...
        if (defined('NGG_PLUGIN_VERSION')) {
            if (version_compare(NGG_PLUGIN_VERSION, '2.2.99') >= 0) {
                return 'photocrati-nextgen_pro_ecommerce#manage_pricelist_redesign';
            }
        }
